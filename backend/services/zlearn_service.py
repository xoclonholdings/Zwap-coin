...
LEARN_MODULES: List[Dict[str, Any]] = [
    {
        "id": "web3-basics",
        "title": "What Is Web3?",
        "level": "beginner",
        ...
        "core": "Web3 is the next layer of the internet where users can do more than just read and interact. They can also hold assets, connect wallets, and participate in systems powered by blockchain.",
        "analogy": "Think of Web2 like renting space in someone else's mall. Web3 is more like entering a city where you can actually own your shop, your keys, and some of the roads you use.",
        "did_you_know": [
        ...
        "content": {
            "overview": "Web3 is a way of building internet products that use blockchains, smart contracts, and community-owned systems.",
            "key_points": [
            ...
            "zwap_context": "In ZWAP! terms, Web3 is not just about holding a token. It is about users interacting with an ecosystem where wallets, rewards, game participation, and token utility can connect through blockchain rails.",
        },
        "trivia": [
            {
                "id": "web3-basics-1",
    ...
    },
    {
        ...
        "content": {
            "overview": "Cryptocurrency is digital money that uses blockchain systems to record and verify transactions.",
            "key_points": [
            ...
            "zwap_context": "ZWAP! is part of a crypto ecosystem, so users need a clear foundation for what digital assets are before they can understand reward flows, swaps, and token utility.",
        },
        "trivia": [
            {
                "id": "edu-crypto-1",
    ...
    },
    {
        ...
        "content": {
            "overview": "A blockchain is a digital ledger that stores information in linked blocks.",
            "key_points": [
            ...
            "zwap_context": "ZWAP! users do not need to be engineers, but they should understand that blockchains are the rails that help digital assets and on-chain systems work.",
        },
        "trivia": [
            {
                "id": "edu-chain-1",
    ...
    },
    {
        ...
        "content": {
            "overview": "A wallet is the tool that helps a user access and control their crypto assets.",
            "key_points": [
            ...
            "zwap_context": "ZWAP! uses wallet-based participation, so wallet education is one of the most important beginner lessons in the app.",
        },
        "trivia": [
            {
                "id": "edu-wallet-1",
    ...
    },
    {
        ...
        "core": "A utility token is a token that is meant to be used. Its job is not just to sit in a wallet. It should unlock value inside a product, platform, or ecosystem.",
        "analogy": "Think of a utility token like a universal pass, arcade card, and rewards key blended together.",
        "did_you_know": [
        ...
        "content": {
            "overview": "A utility token is a digital asset designed to unlock access to features, products, or services inside a specific ecosystem.",
            "key_points": [
                "Utility tokens are meant for use.",
                "Their value is tied to ecosystem function.",
                "They can power rewards, access, and participation.",
            ],
            "zwap_context": "For ZWAP!, utility means the token should connect to real platform behavior such as participation, access, rewards, marketplace actions, and ecosystem features.",
        },
        "trivia": [],
    },
    {
        ...
        "core": "ZWAP! and zPts are connected, but they are not the same thing. zPts are in-app points. ZWAP! is the token layer.",
        "analogy": "Think of zPts like points on your arcade card, while ZWAP! is the actual token rail connected to the wider ecosystem.",
        "did_you_know": [
        ...
        "content": {
            "overview": "ZWAP! is the ecosystem token, while zPts are off-chain in-app points.",
            "key_points": [
            ...
            "zwap_context": "The Learn module should help users understand the path from learning, to participation, to earning, to practical token use inside ZWAP!.",
        },
        "trivia": [],
    },
    {
        ...
        "content": {
            ...
            "zwap_context": "zPts give ZWAP! a smoother reward system by letting the app track progress before token conversion.",
        },
        "trivia": [
            {
                "id": "edu-zpts-1",
                "question": "Are zPts the same as ZWAP!?",
                "options": ["Yes", "No", "They are similar", "Only on Zitizen tier"],
                "answer": "No",
                "difficulty": 1,
            },
            {
            ...
            },
            {
                ...
                "options": ["Yes", "No, they are tracked in the app", "Sometimes", "Only for Zitizen users"],
    ...
    },
    {
        ...
        "content": {
            "overview": "A swap is the exchange of one token or cryptocurrency for another.",
            "key_points": [
            ...
            "zwap_context": "In ZWAP!, swap education helps users understand that exchange functions are based on price movement, fees, and the assets involved.",
        },
        "trivia": [
            {
                "id": "edu-swap-1",
...
def get_module(module_id: str) -> Optional[Dict[str, Any]]:
    safe_id = str(module_id or "").strip()
    return next((module for module in LEARN_MODULES if module["id"] == safe_id), None)
...
def cleanup_expired_trivia_sessions() -> None:
    now = _time.time()
    expired_session_ids = [
        session_id
        for session_id, session in _trivia_sessions.items()
        if session["expires"] < now
    ]

    for session_id in expired_session_ids:
        del _trivia_sessions[session_id]


def create_trivia_session(count: int = 5, difficulty: int = 1) -> Dict[str, Any]:
    cleanup_expired_trivia_sessions()

    safe_count = max(1, min(int(count or 5), 10))
    safe_difficulty = max(1, int(difficulty or 1))

    all_trivia = get_all_trivia()
    filtered = [
        question
        for question in all_trivia
        if int(question.get("difficulty", 1)) <= safe_difficulty + 1
    ]

    selected = random.sample(filtered, min(safe_count, len(filtered)))

    session_id = str(uuid.uuid4())
    _trivia_sessions[session_id] = {
        "questions": {question["id"]: question["answer"] for question in selected},
...
def check_trivia_answer(question_id: str, answer: str, time_taken: float) -> Dict[str, Any]:
    question = next(
        (item for item in get_all_trivia() if item["id"] == question_id),
        None,
    )
    ...
    correct = str(question["answer"]).strip().lower() == str(answer or "").strip().lower()
    safe_time = max(0.0, float(time_taken or 0))
    time_bonus = max(0, 1 - (safe_time / 30)) if correct else 0
