  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-40 bg-[#0a0b1e]/95 backdrop-blur-lg border-b border-cyan-500/10">
      <div className="flex items-center justify-between px-4 py-2 max-w-lg mx-auto">
        ...
        {/* Right side - Balances + Connect Button + Profile (always show connect when not connected) */}
        <div className="flex items-center gap-2">
          {/* Balances - show for connected users or placeholder for non-connected */}
          ...
          {/* Connect Wallet Button - always visible next to balances when not connected */}
          {!walletAddress && (
            <Button
              onClick={() => setIsWalletModalOpen(true)}
              className="h-9 px-3 bg-cyan-500 hover:bg-cyan-600 text-sm"
          ...
          {/* Profile Badge - only for connected users */}
          {walletAddress && (
