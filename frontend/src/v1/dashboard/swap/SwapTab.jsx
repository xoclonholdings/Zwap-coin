...
export default function SwapTab() {
  ...
  const openEmbeddedSwapFlow = useCallback(() => {
    ...
    if (fromToken === toToken) {
      setFeedback({
        type: "error",
        title: "Select a new path",
        message: "Choose two different assets to continue.",
      });
      return;
    }
    ...
    if (fromToken === "ZWAP" && !hasWalletZwap) {
      setFeedback({
        ...
        message: "Claim ZWAP to your wallet before swapping.",
      });
      return;
    }
  ...
  const executePrimaryAction = async () => {
    ...
    if (!fromAmount || !Number.isFinite(amountNum) || amountNum <= 0) {
      setFeedback({
        type: "error",
        title: "Amount needed",
        message: "Enter a valid amount before continuing.",
      });
      return;
    }
