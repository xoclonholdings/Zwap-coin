  ...
  return (
    <div className="relative z-10 flex items-center gap-3 pr-10">
      <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-violet-300/35 bg-violet-400/14 shadow-[0_0_18px_rgba(168,85,247,0.18)]">
        <img src={zapHead} alt="Zap" className="h-8 w-8 object-contain" draggable={false} />
      </div>
...
function GuidanceText({ guidance, onTypingChange }) {
  ...
  return (
    <div className="relative z-10 flex h-full flex-col items-center justify-center gap-1 px-7 pb-7 pt-4 text-center">
      {lines.map((line, index) => {
        ...
        return (
          <div
            ...
            className={
              isEdgeLine
                ? "text-[0.76rem] font-extrabold uppercase leading-[1.02] tracking-[0.05em] text-white drop-shadow-[0_0_10px_rgba(255,255,255,0.16)]"
                : "text-[0.64rem] font-semibold leading-tight tracking-[-0.02em] text-white/70"
            }
          >
            {visibleLine}
            {isTypingLine(index) ? (
...
export default function DashboardWindowZwap({
  ...
  if (isAltView) {
    return (
      <div onClick={handleToggle} role="button" tabIndex={0} className="relative h-full">
        <WindowAltIndicator />
  ...
  return (
    <section onClick={handleToggle} className={shellClassName} role="button" tabIndex={0}>
