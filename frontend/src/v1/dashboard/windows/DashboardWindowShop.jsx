...
export default function DashboardWindowShop({
  ...
  const isUnlocked =
    typeof shopUnlocked === "boolean"
      ? shopUnlocked
      : zptsBalance >= SHOP_UNLOCK_THRESHOLD;

  return (
    <button
      type="button"
      onClick={onOpenTasks}
      aria-label={isUnlocked ? "Open Shop" : "Open Tasks"}
      className="relative w-full overflow-hidden rounded-[1.5rem] border border-white/10 bg-[#0b1220] p-4 text-left shadow-[0_0_24px_rgba(0,0,0,0.25)]"
    >
