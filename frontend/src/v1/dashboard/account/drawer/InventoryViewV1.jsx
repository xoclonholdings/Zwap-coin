...
function ZwapProgressCard({
  zptsBalance = 0,
  isConversionReady = false,
  onOpenSwap,
}) {
  const { progressPercent, progressZone, progressSubtext, progressHint } =
  ...
  return (
    <motion.div
      ...
      className="mb-3 rounded-[28px] border border-white/10 bg-[radial-gradient(circle_at_top,rgba(52,211,153,0.14),transparent_35%),linear-gradient(180deg,rgba(11,24,20,0.96),rgba(7,15,13,0.98))] p-4 shadow-[0_18px_48px_rgba(0,0,0,0.35)]"
    >
      ...
      <div className="mt-4 rounded-[22px] border border-white/10 bg-white/5 p-3">
        ...
        >
          ...
          {isConversionReady ? "Open Swap" : "View Swap"}
        </button>
      </div>
    </motion.div>
  );
